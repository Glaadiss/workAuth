(function(lib) {
	...
	lib.DrawingUtils.drawTriangles = function(g, vertices, triangles, clear, lineThickness, lineColor, lineAlpha, fillColor, fillAlpha) {
		...
		lineColor = lib.defaultValue(lineColor, "rgba(0,200,100,1.0)"); // "#00f6ff"
		lineAlpha = lib.defaultValue(lineAlpha, 0.85);
		fillColor = lib.defaultValue(fillColor, "rgba(255,0,0,1.0)"); // "#00f6ff"
		...
		function setWsp(eyeX, eyeX2, eyeY, eyeY2, noseX, noseX2, noseY, noseY2, midX, midY){
			window.eyeX = eyeX;
			window.eyeX2 = eyeX2;
			window.eyeY = eyeY;
			window.eyeY2 = eyeY2;
			window.noseX = noseX;
			window.noseX2 = noseX2;
			window.noseY = noseY;
			window.noseY2 = noseY2;
			window.midX = midX;
			window.midY = midY;
			console.log( window.eyeX);
		}
		var wspON=0;
		if(counterr>=3){
			counterr = 0;
				var leftX = vertices[31*2];
				var leftY = vertices[31*2+1];
				var rightX = vertices[36*2];
				var rightY = vertices[36*2 +1];
				var noseX = vertices[67*2];
				var noseY = vertices[67*2 + 1];
				var nosL = vertices[39*2];
				var nosR = vertices[43*2];
				var nosL2 = vertices[39*2+1];
				var nosR2= vertices[43*2+1];
				var noseWidth = lineLength(nosL,nosL2,nosR,nosR2);
				var okoOko = lineLength(leftX,leftY,rightX, rightY);
				wspON= noseWidth/okoOko;
				setWsp(leftX, rightX, leftY, rightY, nosL, nosR, nosL2, nosR2, noseX, noseY );
				var leweOkoNos = lineLength(leftX, leftY, noseX, noseY);
				var praweOkoNos = lineLength(rightX, rightY, noseX, noseY);
				//console.log(' Od oka do oka: ' + okoOko + ' Lewe oko do nosa: '+ leweOkoNos +' Prawe oko do nosa: '+ praweOkoNos + ' szerokosc nosa: ' + noseWidth);
				var wsp = okoOko / leweOkoNos;
	...
	lib.DrawingUtils.drawTrianglesAsPoints = function(g, vertices, radius, clear, fillColor, fillAlpha) {
		...
		fillColor = lib.defaultValue(fillColor, "rgba(200,246,255,1.0)"); // "#00f6ff"
